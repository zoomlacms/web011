<%@ page language="C#" autoeventwireup="true" inherits="WebShop_UserCart, App_Web_sbpzdp5g" enableviewstatemac="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE HTML>
<html>
<head>
<title>加入购物车</title>
    ...
    <asp:Repeater ID="Cart_Rep1" runat="server" OnItemDataBound="cartinfo_ItemDataBound">
        <ItemTemplate>
            <ul <%#(Eval("Bindpro","{0}")=="")?"":"style=background-color:#E6E6E6"%> >
                <li class="i10"><input type="checkbox"></li>
                <li class="i0"><a href='Shop.aspx?ItemID=<%#Eval("proid")%>' target="_blank"><%#getproimg(Eval("proid","{0}"))%></a></li>
                ...
                <li class="i3">
                    ...
                    <input onkeydown="if(event.keyCode==13){ keydo('<%#Eval("id") %>');return false;}" id='num<%#Eval("id") %>' value='<%#DataBinder.Eval(Container, "DataItem.pronum", "{0}")%>'  style="width: 30px; height:20px" onblur="keydo('<%#Eval("id") %>')" class="J_input"  />
                    <a href="javascript:;" class="J_add" onclick="setTimeout('keydo(<%#Eval("id") %>)',100);">+</a></li>
                <li class="i4">
                ...
                <li class="i9"><a href="UserCart.aspx?menu=del&cid=<%#Eval("id")%>"  onclick="return confirm('确定？移除后您要从商城重新添加商品!');">移除</a></li>
                <div class="clear"></div>
            </ul>
        </ItemTemplate>
    </asp:Repeater>
