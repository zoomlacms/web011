 <%@ page language="C#" autoeventwireup="true" validaterequest="false" inherits="ZoomLa.WebSite.Manage.Content.EditContent, App_Web_i3fxcuwj" enableviewstatemac="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE HTML>
<html>
...
<script type="text/javascript">
    if (document.documentMode <= 7)// IE 5-7
        {
        alert("警告：当前浏览器版本或渲染模式为IE7，请使用更高浏览器以支持完整应用体验和各项卓越功能!")
        }
</script>
<script type="text/javascript">
function SelectKey() {
	...
	function ShowPhotoPage(url) {
	    window.open('EditImage.aspx?urls='+url+'', 'newWin', 'modal=yes,width=750,height=500,resizable=no,scrollbars=no');
	}
</script>
...
<script type="text/javascript">
    function datecontrol(isWrite) {
        document.getElementById("txtdate").disabled = isWrite;
    }
</script>
<style>
#change{text-align:center;	WIDTH: 100%; display:block;overflow:hidden;margin:0 auto;RIGHT: 0px; BOTTOM: 0px;POSITION:fixed;_POSITION: absolute;z-index:9999;}
#txt_author{width:10%;border:1px solid #8ec1ee;}
#txt_Edit{border:1px solid #8ec1ee;}
.cinput{ margin-top:2px;}
...
.delfiel{ float:left;border: 1px solid #8EC1EE;}
#LinkButton1{ margin-left:10px; margin-top:8px;}
#LinkButton1:hover{color:currentColor;}
</style>
</head>
<body>
...
<span style="float: right; padding-right: 10px;">
    ...
    <%=GetOper()%>
    <a onclick="opentitle('EditNode.aspx?NodeID=<%=this.NodeID %>','配置本节点')"
                href="javascript:void(0);" title="配置本节点">
                <%=lang.LF("配置本节点")%></a></span> 
后台管理 &gt;&gt; <a href="ContentManage.aspx">内容管理</a>&gt;&gt;<asp:Label ID="Label2" runat="server"  Text="Label"></asp:Label>&gt;&gt;编辑内容</div>
...
<table style="width: 100%; margin: 0 auto;" cellpadding="2" cellspacing="1" class="border">
	<tbody id="Tabs0">
		...
		<tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
			...
			<td class="tdbg">
				<asp:TextBox ID="txtTitle" CssClass="l_input" onblur="getstrKeys()" onkeyup="isgoEmpty('txtTitle','span_txtTitle');Getpy('txtTitle','PYtitle')"
					runat="server" Text='' MaxLength="255" Style="width: 41%;"></asp:TextBox>
		...
		<tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
			...
			<td class="bqright">
				<asp:TextBox ID="TxtTagKey" CssClass="l_input" runat="server" Text='' Width="41%"></asp:TextBox>
				&nbsp;<span style="color: #0000ff"><input id="Button7" type="button" value="提取关键字"
					onclick="getstrKeys()" class="C_input" style="width: 110px;margin-left:8px;" />[</span><a href="#"
		...
		<tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
			...
			<td class="bqright">
				<asp:TextBox ID="Subtitle" runat="server" Text='' Width="41%" CssClass="l_input" TextMode="SingleLine"></asp:TextBox>
			</td>
		</tr>
	...
	<tbody id="Tabs1" style="display: none">
		...
		<tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
			...
			<td class="bqright">
				<asp:TextBox ID="txtAddTime" CssClass="l_input" runat="server" onclick="WdatePicker({ dateFmt: 'yyyy-MM-dd HH:mm:ss' });"></asp:TextBox>
			</td>
		</tr>
		<tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
			<td class="tdbgleft" style="width: 20%" align="right">
			...
			<td class="bqright">
                <asp:TextBox ID="txtdate" class="l_input" Style="font-size: 9pt; height: 15px" MaxLength="20" runat="server" onclick="WdatePicker({ dateFmt: 'yyyy-MM-dd HH:mm:ss' });"></asp:TextBox>
		...
		<tr class="tdbg" id="outpdf" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
			<td class="tdbgleft" style="width: 20%" align="right">
                 <asp:Button runat="server" ID="makePDFBtn" Text="生成PDF" CssClass="C_input" OnClick="makePDFBtn_Click"/>
				&nbsp;
			</td>
			<td class="bqright">
        ...
        <tr class="tdbg" id="outword" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
            <td class="tdbgleft" style="width: 20%" align="right">
               <asp:Button runat="server" ID="makeWordBtn" Text="生成Word" CssClass="C_input" OnClick="makeWordBtn_Click"/>&nbsp;&nbsp;
            </td>
            <td class="bqright">
	...
	<tbody id="Tabs3" style="display: none">
		...
		<tr id="bidmoneytable" class="tdbg" onmouseover="this.className='tdbgmouseover'"
			...
			<td class="bqright">
				<asp:TextBox ID="proweek" runat="server" Width="30px" CssClass="l_input">0</asp:TextBox>天
			</td>
		</tr>
	</tbody>
	<tr>
		<td colspan="2" >
			...
			<asp:Button ID="EBtnSubmit" CssClass="C_input cinput" Text="保存项目" runat="server" OnClick="EBtnSubmit_Click"  />&nbsp;
			<asp:Button ID="Button1"    CssClass="C_input cinput" Text="添加为新项目" runat="server" OnClick="Button1_Click" />&nbsp;
			<asp:Button ID="BtnBack"    CssClass="C_input cinput" Text="返　回"  runat="server" OnClick="BtnBack_Click" UseSubmitBehavior="False" CausesValidation="False" />
            <asp:Button ID="BtnDoc" CssClass="C_input cinput" runat="server"  OnClientClick="return generate('docTitle','docParamValue');" Text="重新生成文档"  Visible="false"/>
...
<script type="text/javascript">
	...
	function Uifra(width, height, title, type, id) {
		...
		var cid = vas[0];
		var srcva;
		srcva = "<%=CustomerPageAction.customPath2%>Charts/Show.aspx?Did=" + cid;
	...
	function Uchart(id) {
		var hid = id.substr(5);
		window.open('<%=CustomerPageAction.customPath2%>Flex/CAddChart.aspx?cdid=' + id + '&ChartID=' + $("#" + hid).val(), 'newWin', 'modal=yes,width=800,height=500,resizable=yes,scrollbars=yes');
	}
