<%@ page language="C#" autoeventwireup="true" inherits="manage_AddOn_Projects, App_Web_hc0q2vuk" enableviewstatemac="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE HTML>
<html>
<head runat="server">
<title><%=lang.LF("查看项目")%></title>
...
<body>
  ...
  <%} %><span><%=lang.LF("后台管理")%></span>&nbsp;&gt;&gt;&nbsp;<%=lang.LF("CRM应用")%></span>&nbsp;&nbsp;<span><a
            href="Projects.aspx"><%=lang.LF("项目管理")%></a></span>&nbsp;<a
                href="AddProjects.aspx" style="color: Red">[<%=lang.LF("新增项目")%>]</a>
    </div>
    <div class="clearbox">
    </div>
    <div class="line">
        ...
        <asp:TextBox ID="TxtADName" runat="server" class="l_input" onclick="if(this.value=='关键字'){this.value=this.value=='关键字'?'':this.value;}"
            onblur="if(this.value==''){ this.value=this.value==''?'关键字':this.value;}">关键字</asp:TextBox>
             &nbsp;<%=lang.LF("时间")%>：<select id="timeval" runat="server" onchange="changeTime(this.value,'-1')">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr align="center">
            <td id="TabTitle0" class="titlemouseover" onclick="ShowTabss(0,0)">
                <%=lang.LF("所有")%>
            </td>
            <td id="TabTitle1" class="tabtitle" onclick="ShowTabss(1,0)">
                <%=lang.LF("未审核")%>
            </td>
            <td id="TabTitle2" class="tabtitle" onclick="ShowTabss(2,0)">
                <%=lang.LF("已审核")%>
            </td>
            <td id="TabTitle3" class="tabtitle" onclick="ShowTabss(3,0)">
                <%=lang.LF("未启动")%>
            </td>
            <td id="TabTitle4" class="tabtitle" onclick="ShowTabss(4,0)">
                <%=lang.LF("已启动")%>
            </td>
            <td id="TabTitle5" class="tabtitle" onclick="ShowTabss(5,0)">
                <%=lang.LF("已完成")%>
            </td>
            <td id="TabTitle6" class="tabtitle" onclick="ShowTabss(6,0)">
                <%=lang.LF("存档")%>
            </td>
            <td>
                &nbsp;
                    ...
                    <tr id='<%#Eval("ID") %>' class="tdbg" height="25px" align="center" onmouseover="this.className='tdbgmouseover'"
                        ...
                        <td>
                            ...
                            <asp:LinkButton ID="LinkButton2" runat="server" CommandArgument='<%# Eval("ID")%>'
                                CommandName="manage"><%=lang.LF("修改")%></asp:LinkButton>
                            <asp:LinkButton ID="LinkButton3" runat="server" CommandArgument='<%# Eval("ID")%>'
                                CommandName="del" OnClientClick="if(!this.disabled) return confirm('确实要删除吗？');"><%=lang.LF("删除")%></asp:LinkButton>
                            <asp:LinkButton ID="LbtnComments" runat="server" CommandArgument='<%# Eval("ID")%>'
                                CommandName="Comments"><%=lang.LF("评论")%></asp:LinkButton>
                        </td>
                    </tr>
                </ItemTemplate>
            </asp:Repeater>
