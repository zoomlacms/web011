<%@ page language="C#" autoeventwireup="true" inherits="manage_UserShopManage_ShopScutcheon, App_Web_2pfg00wt" enableviewstatemac="false" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
        ...
        <asp:Repeater ID="Trademarklist" runat="server">
            <ItemTemplate>
                <tr ondblclick="editScutcheon('<%# Eval("id") %>')">
                    ...
                    <td class="text-center">&nbsp; <a href="AddShopBrand.aspx?menu=edit&id=<%#Eval("id") %>">修改</a> <%#showstop(DataBinder.Eval(Container, "DataItem.id", "{0}"))%> <%#showtop(DataBinder.Eval(Container, "DataItem.id", "{0}"))%> <%#showjian(DataBinder.Eval(Container, "DataItem.id", "{0}"))%> <a href="?menu=delete&id=<%#Eval("id") %>" onclick="return confirm('不可恢复性删除数据,你确定将该数据删除吗？');">删除</a></td>
                </tr>
            </ItemTemplate>
        </asp:Repeater>
        <tr>
