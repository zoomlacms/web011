<%@ page language="C#" autoeventwireup="true" enableeventvalidation="false" inherits="ZoomLa.WebSite.Manage.Config.UserConfig, App_Web_gsi5yeuj" validaterequest="false" enableviewstatemac="false" masterpagefile="~/Manage/I/Default.master" viewStateEncryptionMode="Never" %>
<asp:Content runat="server" ContentPlaceHolderID="head">
<title>用户参数</title>
</asp:Content>
<asp:Content runat="server" ContentPlaceHolderID="Content">
<ul class="nav nav-tabs">
<li class="active"><a href="#tab0" data-toggle="tab" onclick="show1(0)">基本参数</a></li>
<li><a href="#tab1" data-toggle="tab" onclick="show1(1)">积分设置</a></li>
<li><a href="#tab2" data-toggle="tab" onclick="show1(2)">期限金额</a></li>
<li><a href="#tab3" data-toggle="tab" onclick="show1(3)">点券设置</a></li>
<li><a href="#tab4" data-toggle="tab" onclick="show1(4)">IP限制</a></li>
</ul>
<table class="table table-striped table-bordered table-hover myform-control">
	<tbody id="Tabs0">
		...
		<tr>
			...
			<td>
				<asp:RadioButtonList ID="radioUserID" runat="server" RepeatDirection="Horizontal">
					<asp:ListItem>是</asp:ListItem>
					<asp:ListItem>否</asp:ListItem>
				</asp:RadioButtonList></td>
		</tr>
		<tr>
			<td><strong>开启注册手机验证码校验：</strong></td>
			<td>
				<asp:RadioButtonList ID="radioMobile" runat="server" RepeatDirection="Horizontal">
					<asp:ListItem>是</asp:ListItem>
					<asp:ListItem>否</asp:ListItem>
				</asp:RadioButtonList></td>
		</tr>
		...
		<tr>
			...
			<td>
				<asp:RadioButtonList ID="rdoEmailCheck" runat="server" RepeatDirection="Horizontal">
					<asp:ListItem>是</asp:ListItem>
					<asp:ListItem>否</asp:ListItem>
				</asp:RadioButtonList>
			</td>
		...
		<tr>
			...
			<td>
				<asp:RadioButtonList ID="radioEmail" runat="server" RepeatDirection="Horizontal">
					<asp:ListItem>是</asp:ListItem>
					<asp:ListItem>否</asp:ListItem>
				</asp:RadioButtonList></td>
		</tr>
		...
		<tr>
			<td><strong>是否允许一个Email注册多个会员：</strong></td>
			<td>
		...
		<tr>
			<td><strong>是否允许多人同时使用同一会员帐号登陆：</strong></td>
			<td>
        ...
        <tr>
            <td><strong>会员生成邀请码的最大数值：</strong></td>
            <td>
                <asp:TextBox ID="InviteCode_T" runat="server" class="form-control"></asp:TextBox>
				...
				<asp:CompareValidator ID="CompareValidator2" runat="server" ControlToValidate="InviteCode_T" ErrorMessage="最少字符数必须大于等于1" Operator="GreaterThanEqual" Type="Integer" ValueToCompare="1" Display="Dynamic" SetFocusOnError="true"></asp:CompareValidator></td>
        </tr>
		<tr>
		...
		<tr>
			<td><strong>新会员注册时发送的验证邮件内容：<br />
			</strong>邮件内容支持HTML，邮件内容中可用标签如下：<br />
				<span>{$CheckNum}</span>：验证码<br />
				<span>{$CheckUrl}</span>：验证地址 </td>
			<td>
				<asp:TextBox ID="TextBox12" runat="server" Rows="6" TextMode="MultiLine" Width="300px" class="form-control"></asp:TextBox></td>
		</tr>
		<tr>
			<td><strong>注册成功时发送的提醒邮件内容：<br />
			</strong>邮件内容支持HTML，邮件内容中可用标签如下：<br />
				<span>{$UserName}</span>：用户名<br />
				<span>{$UserPwd}</span>：密码 </td>
			<td>
				<asp:TextBox ID="txtEmailTell" runat="server" Rows="6" TextMode="MultiLine" Width="300px" class="form-control"></asp:TextBox></td>
		</tr>
		<tr>
			<td><strong>使用邮箱注册自动验证邮件内容：<br />
			</strong>邮件内容支持HTML，邮件内容中可用标签如下：<br />
			...
			<td>
				<asp:TextBox ID="txtEmailRegInfo" runat="server" Rows="6" TextMode="MultiLine" Width="300px" class="form-control"></asp:TextBox></td>
		</tr>
		<tr>
			<td><strong>使用邮箱找回密码邮件内容：<br />
			</strong>邮件内容支持HTML，邮件内容中可用标签如下：<br />
			...
			<td>
				<asp:TextBox ID="txtGetPassword" runat="server" Rows="6" TextMode="MultiLine" Width="300px" class="form-control"></asp:TextBox></td>
		</tr>
		<tr>
	...
	<tbody id="Tabs1"  class="table" style="display:none;width:100%;">
		<tr	runat="server" id="PresentExpPerLogin" >
			<td style="width:10%;"><strong>会员每登录一次奖励的积分：</strong><br />
				一天只计算一次 </td>
			<td style="width:60%;">
	...
	<tbody id="Tabs2" style="display: none;width:100%;">
		<tr>
			<td style="width:10%;">会员找回密码的方式：</td>
			<td>
		...
		<tr
			...
			<td><strong>会员的资金与点券的兑换比率：</strong></td>
			<td>每
	  ...
	  <asp:RegularExpressionValidator ID="ValeMoneyExchangePoint" runat="server" ControlToValidate="TxtMoneyExchangePoint"
		...
		<tr
			...
			<td><strong>会员的资金与有效期的兑换比率：</strong></td>
			<td>每
	  ...
	  <asp:RegularExpressionValidator ID="ValeMoneyExchangeValidDay" runat="server" ControlToValidate="TxtMoneyExchangeValidDay"
		...
		<tr runat="server">
			<td><strong>会员的资金与虚拟金币的兑换比率：</strong></td>
			<td>每
	  ...
	  <asp:RegularExpressionValidator ID="RegularExpressionValidator3" runat="server" ControlToValidate="txtMoneyExchangeDummyPurse"
		...
		<tr
			...
			<td><strong>会员的积分与点券的兑换比率：</strong></td>
			<td>每
	  ...
	  <asp:RegularExpressionValidator ID="ValgUserExpExchangePoint" runat="server" ControlToValidate="TxtUserExpExchangePoint"
		...
		<tr
			...
			<td><strong>会员的积分与有效期的兑换比率：</strong></td>
			<td>每
	  ...
	  <asp:RegularExpressionValidator ID="ValgUserExpExchangeValidDay" runat="server" ControlToValidate="TxtUserExpExchangeValidDay"
		...
		<tr
			runat="server" id="Tr4">
			<td><strong>会员的积分与资金的兑换比率：</strong></td>
			<td>每
	  ...
	  <asp:RegularExpressionValidator ID="RegularExpressionValidator1" runat="server" ControlToValidate="TxtCUserExpExchangePoints"
		...
		<tr
			runat="server" id="Tr8">
			<td><strong>会员的积分与银币的兑换比率：</strong></td>
			<td>每
	  ...
	  <asp:RegularExpressionValidator ID="RegularExpressionValidator5" runat="server" ControlToValidate="TxtCUserExpExchangeExp"
		...
		<tr>
			...
			<td>
				<select id="selPunch" runat="server" onchange="PunchCha()">
	...
	<tbody id="Tabs3" style="display: none;width:100%;">
		<tr runat="server" id="PointName">
			<td style="width:10%;"><strong>点券的名称：</strong><br />
				例如：点券、金币、拍点 </td>
	...
	<tbody id="Tabs4" style="display: none; width:100%;">
		<tr>
			<td style="width:10%;"><strong>是否对同一IP注册进行时间间隔限制：</strong></td>
			<td>
		...
		<tr>
			<td align="right" style="height: 23px" valign="middle"><strong>注册间隔限制时间：</strong></td>
			<td>
		...
		<tr>
			<td align="right" style="height: 23px" valign="middle"><strong>是否设置限制的IP段：</strong></td>
			<td>
		...
		<div id="IPpart" runat="server">
			<tr>
				<td align="right" style="height: 23px" valign="middle"><strong>设置限制的IP段：</strong></td>
				<td>
	  ...
	  <asp:TextBox ID="txtEndIP" class="form-control" runat="server" Width="150px"></asp:TextBox>
        ...
        <tr id="#save">
		<td class="text-center" colspan="2"><asp:Button ID="Button1" runat="server" Text="保存设置" OnClick="Button1_Click" class="btn btn-primary" /></td>
	</tr>
	...
	function show1(SID) {
		...
		if (SID != tID) {
			...
			if (document.getElementById(arrTabs[SID].toString()) != null)
			    document.getElementById(arrTabs[SID].toString()).style.display = "inherit";
			tID = SID;
		}
