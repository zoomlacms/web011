<%@ page language="C#" autoeventwireup="true" inherits="Manage_I_Exam_NewsDiv, App_Web_fk1hbgsf" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
    ...
    <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" AllowPaging="true" PageSize="10" EnableTheming="False"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作">
                <ItemTemplate>
                    <a href="/Rss/News/ViewPublish.aspx?Pid=<%#Eval("Pid") %>&ID=<%#Eval("ID") %>" target="_blank" title="前台浏览">浏览</a><span class="sperspan">|</span>
                    <a href="Publish.aspx?Pid=<%#Eval("Pid") %>&ID=<%#Eval("ID") %>" target="_parent">修改</a><span class="sperspan">|</span>
                    <asp:LinkButton runat="server" CommandArgument='<%#Eval("ID") %>' CommandName="Del2" OnClientClick="return confirm('确定要删除该版面吗?');" >删除</asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>
