<%@ page language="C#" autoeventwireup="true" inherits="Manage_I_Content_ShowContent, App_Web_gqexlk3x" masterpagefile="~/Manage/I/Default.master" clientidmode="Static" validaterequest="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
        ...
        <ZL:ExGridView ID="Egv"  runat="server" AutoGenerateColumns="False" DataKeyNames="CommentID" PageSize="20" OnPageIndexChanging="Egv_PageIndexChanging" IsHoldState="false" OnRowCommand="Egv_RowCommand" Width="100%" Height="20px" AllowPaging="True" AllowSorting="True" CellPadding="2" CellSpacing="1" BackColor="White" ForeColor="Black" CssClass="table table-bordered" EnableTheming="False" GridLines="None" EnableModelValidation="True" EmptyDataText="无评论信息!!" >
            <Columns>
                ...
                <asp:TemplateField HeaderText="操作">
                    <ItemTemplate>
                        <a href="CommentView.aspx?CommentID=<%#Eval("CommentID") %>&modeid=<%=HdnModelID.Value %>&GID=<%=HiddenGID.Value %>">预览</a>
                        <a href="EditComment.aspx?CommentID=<%#Eval("CommentID") %>&modeid=<%=HdnModelID.Value %>&GID=<%=HiddenGID.Value %>">修改</a>
                        <asp:LinkButton ID="LinkButton1" CommandName="audit" CommandArgument='<%#Eval("CommentID") %>' runat="server">审核操作</asp:LinkButton>
