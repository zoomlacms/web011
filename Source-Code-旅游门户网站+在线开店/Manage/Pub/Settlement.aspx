<%@ page language="C#" autoeventwireup="true" inherits="manage_Pub_Settlement, App_Web_bfm3gvdt" enableviewstatemac="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE HTML>
<html>
...
<div id="nav_box">
<div class="r_navigation">后台管理&gt;&gt;互动管理&gt;&gt;竞标管理&gt;&gt;<a href="Settlement.aspx">结算中心</a></div>
</div>
<div class="h_30"></div>
			...
			<table  cellpadding="2" border="0" cellspacing="1" style="background:#fff; margin-left:5px;" class="l_input">
                <tr>
                    <td>
                        ...
                        <asp:Button ID="Button1" runat="server" Text="" class="C_sch" onclick="Button1_Click" />
                    </td>
                </tr>
			</table>
		</td>
...
<table width="100%" cellspacing="1" cellpadding="0" class="border">
	...
	<tbody>
		<tr class="gridtitle" align="center" style="height: 25px;">
			...
			<td width="15%"><strong>结算编号</strong></td>
			<td width="10%"><strong>竞标价格</strong></td>
			<td width="10%"><strong>中标分成</strong></td>
			<td width="10%"><strong>达标分成</strong></td>
			<td width="10%"><strong>网站商分成</strong></td>
			<td width="10%"><strong>结算时间</strong></td>
        ...
        <asp:Repeater ID="Repeater1" runat="server">
            <ItemTemplate>
				<tr class="tdbg" align="center" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'" style="height: 25px;">
					<td><%#Eval("ID") %></td>
					<td><%#Eval("SettlementTitle")%></td>
					<td align="center"><%#Eval("SettlementAllMoney","{0:F2}")%></td>
					<td align="center"><%#Eval("SuccessfulMoney", "{0:F2}")%></td>
					<td><%#Eval("ComplianceMoney", "{0:F2}")%></td>
					<td><%#Eval("WebSiteMoney", "{0:F2}")%></td>
					<td><%#Eval("SettlementTime")%></td>
					<td><a href="?menu=delete&id=<%#Eval("ID") %>" onclick="return confirm('确实要删除吗?');">删除</a></td>
				</tr>
			</ItemTemplate>
		</asp:Repeater>
		<tr class="tdbg">
