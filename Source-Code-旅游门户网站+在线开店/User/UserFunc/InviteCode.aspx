<%@ page language="C#" autoeventwireup="true" inherits="User_UserFunc_InviteCode, App_Web_ittbslzb" masterpagefile="~/User/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    ...
    <div class="input-group nav_searchDiv" style="display:inline;">
        <asp:TextBox runat="server" ID="searchText" class="form-control" placeholder="请输入需要搜索的邀请码" />
        <span class="input-group-btn" style="float:left;">
            <asp:LinkButton runat="server" CssClass="btn btn-default" ID="searchBtn" OnClick="searchBtn_Click"><span class="glyphicon glyphicon-search"></span></asp:LinkButton>
        </span>
    </div>
            <span style="margin-left:50px;line-height:30px;color:green;">您当前有<asp:Label runat="server" ID="CodeCount_L" />个有效的邀请码,最多拥有<%:maxcount %>个</span>
            ...
            <asp:TemplateField HeaderText="操作">
                <ItemTemplate>
                    <asp:LinkButton runat="server" CommandName="Del2" CommandArgument='<%#Eval("ID") %>' OnClientClick="return confirm('你确定要移除吗!');" ToolTip="移除">
                              <span class="glyphicon glyphicon-trash" style="color:#7D98A1;"></span></asp:LinkButton>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
