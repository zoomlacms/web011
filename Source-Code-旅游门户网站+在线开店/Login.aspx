<%@ page language="C#" autoeventwireup="true" inherits="ZoomLa.WebSite._Login, App_Web_1s3tbu0c" enableviewstatemac="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE HTML>
<html>
...
function loginSec(obj) {
    ...
    var str = '<%= Server.HtmlEncode(Request["page"])%>';
    if (obj == 0)
    {
...
<style>
     #login,#logged { padding-left:40px; padding-top:30px;}
    .l_input {}
</style>
</head>
<body>
...
<div id="login">
    <ul>
      <li>用户名：<asp:TextBox ID="TxtUserName" class="l_input" Width="200" runat="server"></asp:TextBox></li>
      <li>密　码：<asp:TextBox ID="TxtPassword" class="l_input" runat="server" Width="200" TextMode="Password"></asp:TextBox></li>
...
function logout(){
	ZoomLa.ajaxlogout(function(){
	    window.location = "/login.aspx?Style=" +Style + "&page=<%= Server.HtmlEncode(Request["page"])%>";
	});
}
	//登录样式 1为纵向 2为横向
    var Style = "<%=style%>";
