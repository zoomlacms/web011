<%@ page language="C#" autoeventwireup="true" inherits="MIS_OA_Mail_SelGroup, App_Web_mb5viozi" clientidmode="Static" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<head runat="server">
    ...
    <title>用户组选择</title>
    <link type="text/css" href="/Mis/OA/CSS/ZLOA.css" rel="stylesheet" />
    ...
    <style type="text/css">
        ...
        * html .fixed {position: absolute;bottom: auto;
                ...
                <ContentTemplate>
                    <div style="float: left; margin-left: 10px; width: 600px; top: 2px;">
                        <div style="margin-bottom: 5px;">
                            ...
                            <asp:Button runat="server" ID="keyBtn" Text="搜索" CssClass="btn-primary" OnClick="keyBtn_Click" OnClientClick="return Search();" Height="34" />
                            <button type="button" id="clearBtn" onclick="ClearAll();" class="btn-primary" style="height:34px;">清除选择</button>
                            <asp:Button runat="server" ID="showAll_Btn" Text="全部用户" CssClass="btn-primary" Visible="false" OnClick="showAll_Btn_Click" Height="34" />
                            <input type="button" id="AllInfo_sure_btn" value="确定" onclick="AllInfo_sureF();" class="btn-important" />
                            <input type="button" value="关闭" onclick="$(parent.document).find('#userdiv').hide();" class="btn-primary" style="height:34px;float:right;margin-right:5px;"/>
                        </div>
                        <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" AllowPaging="true" PageSize="10" EnableTheming="False" CssClass="table border" EmptyDataText="当前没有信息!!"
                            ...
                            <Columns>
                                <asp:TemplateField HeaderText="<input id='chkAll' onclick='checkAll(this)' type='checkbox' />" >
                                    <ItemTemplate>
                                        <input type="checkbox" <%#IsChecked(Eval("UserID")+":"+Eval("UserName")) %>  name="idChk" value="<%#Eval("UserID") %>" userinfo="<%#Eval("UserID")+":"+Eval("UserName") %>" onclick="SaveHide(this);" />
