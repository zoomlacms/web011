...
function PopImage(divID, path, width, heigh) {
    ...
    if (document.getElementById(path).value.indexOf("http:") > -1) {
        image.src = document.getElementById(path).value;//path; 
    }
    else {
        image.src = "/UploadFiles/" + document.getElementById(path).value;//path; 
    }
    $(image).parent().show();
}
//自动缩放图片， 居中显示
